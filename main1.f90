...
program hello
    use share
    implicit none
    external fcn,hybrd1,fcn1,lmdif1,two_pol
    double precision :: tol
    double precision :: x(2),fvec(2),res(2,2)
    integer :: i,info
    call two_pol(1.d0,res)
    print*,res(2,1)
    print*,res(2,2)
...
subroutine two_pol(x,res)
    use share
    implicit none
    external fcn,hybrd1
    double precision, intent(in) :: x
    double precision,intent(out) :: res(2,2)
    double precision :: init(2),fvec(2),x_1000(30),first_pol(2),x_100(30),x1_1000(10)
    double precision :: tol
    integer :: info,i
    chi = x
    info = 0
    init(1) = 0.d0
    init(2) = 0.d0
    call hybrd1 (fcn, 2, init, fvec, tol, info )
    res(1,1) = init(1)
    res(1,2) = init(2)
    first_pol(1) = init(1)
    first_pol(2) = init(2)
    ...
    if (chi < 1001 .and. chi >=  80) then
        do i = 1,10
            init(1) = 10.d0
            init(2) = -100.d0*i
            call hybrd1 (fcn, 2, init, fvec, tol, info )
            if (norm(init,first_pol) > 1 .and. (fvec(1)**2+fvec(2)**2) < 1 ) then
               exit
            end if
    ...
    if (chi < 80 .and. chi >= 30) then
        do i = 1,30
            init(1) = 10.d0
            init(2) = -(1000.d0/30.d0)*i
            call hybrd1 (fcn, 2, init, fvec, tol, info )
            if (norm(init,first_pol) > 1 .and. (fvec(1)**2+fvec(2)**2) < 1 ) then
               exit
            end if
    ...
    if (chi < 30 .and. chi >= 2  ) then
        do i = 1,30
            init(1) = 10.d0
            init(2) = -(100.d0/30.d0)*i
            call hybrd1 (fcn, 2, init, fvec, tol, info )
            if (norm(init,first_pol) > 1 .and. (fvec(1)**2+fvec(2)**2) < 1 ) then
               exit
            end if
    ...
    double precision function norm (x1,x2)
        double precision :: x1(2),x2(2)
        norm = (x1(1)-x2(1))**2 + (x1(2)-x2(2))**2
...
subroutine  fcn1(m,n,x,fvec,iflag)
    ...
    l1 = x(1)
    l2 = x(2)
    fvec(1) = l1 - pi_1_re(l1,l2,chi)
    fvec(2) = l2 - pi_1_im(l1,l2,chi)
    ...
    double precision function last_integrand(x)
    use share
    double precision :: x,res,epsrel,epsabs,abserr
    integer :: neval,ier,limit,lenw,last
    integer, dimension(1000) :: iwork
    double precision, dimension(4000) :: work
    limit = 50
    lenw = 4*limit
    epsabs = 1.49e-8
    res = 0
    ...
    last = 0
    u = x
    call dqagi(integrand_re_tau,0.d0,1,epsabs,epsrel,res,abserr,neval,ier,limit,lenw,last,iwork,work)
    last_integrand = res*(((1.d0-u)**(1.d0/3.d0))*(2.d0*u*u+6.d0*(1.d0-u)*(1.d0-u))/((u*u+4.d0*(1.d0-u)*(1.d0-u))**(13.d0/6.d0)))
    end function last_integrand

    double precision function integrand_re_tau(t)
    use share
    double precision :: y1,y2,z1,z2,a,pi,t
    pi = 4.d0*datan(1.d0)
    y1 = l1 -dsqrt(3.d0)*l2
    y2 = l1 +l2/dsqrt(3.d0)
    z1 = (u**2 +(4.d0-y1)*((1.d0-u)**2))/(((chi*(1.d0-u)**2)**(2.d0/3.d0))*((u**2+4.d0*((1.d0-u)**2))**(1.d0/3.d0)))
    z2 = (u**2 +(4.d0-y2)*((1.d0-u)**2))/(((chi*(1.d0-u)**2)**(2.d0/3.d0))*((u**2+4.d0*((1.d0-u)**2))**(1.d0/3.d0)))
    a = dcos(pi/3.d0+sqrt(3.d0)*z2*t/2.d0)
    integrand_re_tau = t*a*dexp(-z1*t/2.d0-t*t*t/3.d0)
    end function integrand_re_tau

    double precision function pi_1_re(p1,p2,x)
    use share
    double precision :: p1,p2,x,res,epsrel,epsabs,abserr,pi
    integer :: neval,ier,limit,lenw,last,key
    integer, dimension(100) :: iwork
    double precision, dimension(400) :: work
    pi = 4.d0*datan(1.d0)
    limit = 100
    lenw = 4*limit
    epsabs = 1.49e-8
    ...
    l2 = p2
    chi = x
    call dqags(last_integrand,0.d0,1.d0,epsabs,epsrel,res,abserr,neval,ier,limit,lenw,last,iwork,work)
    pi_1_re = res*(4.d0*(chi**(2.d0/3.d0))/(3.d0*137.d0*pi))
    end function pi_1_re

    double precision function integrand_im_tau(t)
    use share
    double precision :: y1,y2,z1,z2,a,pi,t
    pi = 4.d0*datan(1.d0)
    y1 = l1 -dsqrt(3.d0)*l2
    ...
    z1 = (u**2 +(4.d0-y1)*((1.d0-u)**2))/(((chi*(1.d0-u)**2)**(2.d0/3.d0))*((u**2+4.d0*((1.d0-u)**2))**(1.d0/3.d0)))
    z2 = (u**2 +(4.d0-y2)*((1.d0-u)**2))/(((chi*(1.d0-u)**2)**(2.d0/3.d0))*((u**2+4.d0*((1.d0-u)**2))**(1.d0/3.d0)))
    a = -dsin(pi/3.d0+sqrt(3.d0)*z2*t/2.d0)
    integrand_im_tau = t*a*dexp(-z1*t/2.d0-t*t*t/3.d0)
    end function integrand_im_tau

    double precision function last_integrand_im(x)
    use share
    double precision :: x,res,epsrel,epsabs,abserr
    integer :: neval,ier,limit,lenw,last
    integer, dimension(1000) :: iwork
    ...
    last = 0
    u = x
    call dqagi(integrand_im_tau,0.d0,1,epsabs,epsrel,res,abserr,neval,ier,limit,lenw,last,iwork,work)
    last_integrand_im = res*(((1.d0-u)**(1.d0/3.d0))*(2.d0*u*u+6.d0*(1.d0-u)*(1.d0-u))/((u*u+4.d0*(1.d0-u)*(1.d0-u))**(13.d0/6.d0)))
    end function last_integrand_im

    double precision function pi_1_im(p1,p2,x)
    use share
    double precision :: p1,p2,x,res,epsrel,epsabs,abserr,pi
    integer :: neval,ier,limit,lenw,last,key
    integer, dimension(100) :: iwork
    ...
    l2 = p2
    chi = x
    call dqags(last_integrand_im,0.d0,1.d0,epsabs,epsrel,res,abserr,neval,ier,limit,lenw,last,iwork,work)
    pi_1_im = res*(4.d0*(chi**(2.d0/3.d0))/(3.d0*137.d0*pi))
    end function pi_1_im
...
subroutine  tech(k1,k2,x,res)
    ...
    l1 = k1
    l2 = k2
    res = (l1 - pi_1_re(l1,l2,chi))*(l1 - pi_1_re(l1,l2,chi)) + (l2 - pi_1_im(l1,l2,chi))*(l2 - pi_1_im(l1,l2,chi))
...
!
!    Input, real ( kind = rk ) X(N), the point at which the jacobian is to be
!    evaluated.
!
!    Input, real ( kind = rk ) FVEC(M), is used only when MODE = 2.
!    In that case, it should contain the function values at X.
!
!    Input, real ( kind = rk ) FJAC(LDFJAC,N), an M by N array.  When MODE = 2,
...
!
!    Output, real ( kind = rk ) XP(N), on output with MODE = 1, is a neighboring
!    point of X, at which the function is to be evaluated.
!
!    Input, real ( kind = rk ) FVECP(M), on input with MODE = 2, is the function
!    value at XP.
!
!    Input, integer MODE, should be set to 1 on the first call, and
!    2 on the second.
!
!    Output, real ( kind = rk ) ERR(M).  On output when MODE = 2, ERR contains
...
!
!    Input, integer NPRINT, enables controlled printing of
...
!    Output, real ( kind = rk ) FVEC(N), the functions evaluated at the output X.
!
!    Output, real ( kind = rk ) FJAC(LDFJAC,N), an N by N matrix, containing
...
!    with IFLAG = 2.
!
!    Output, real ( kind = rk ) R(LR), the upper triangular matrix produced
...
!
!    Output, integer NFEV, the number of calls to FCN with
!    IFLAG = 1.
!
!    Output, integer NJEV, the number of calls to FCN with
!    IFLAG = 2.
!
!    Output, integer IPVT(N), defines a permutation matrix P
...
!
!    Output, real ( kind = rk ) QTF(N), contains the first N elements of Q'*FVEC.
!
  implicit none
...
!    Output, real ( kind = rk ) FVEC(M), the functions evaluated at the output X.
!
!    Output, real ( kind = rk ) FJAC(LDFJAC,N), an N by N array.  The upper
...
!    LDFJAC must be at least N.
!
!    Input, real ( kind = rk ) FTOL.  Termination occurs when both the actual and
...
!    be nonnegative.
!
!    Input, integer MAXFEV.  Termination occurs when the number
...
!    Input, integer N, the order of R.
!
!    Input/output, real ( kind = rk ) R(LDR,N).  On input the upper triangular
...
!
!    Input, real ( kind = rk ) W(N), the row vector to be added to R.
!
!    Input/output, real ( kind = rk ) B(N).  On input, the first N elements
!    of the vector C.  On output the first N elements of the vector Q'*C.
!
!    Input/output, real ( kind = rk ) ALPHA.  On input, the (N+1)-st element
!    of the vector C.  On output the (N+1)-st element of the vector Q'*C.
!
!    Output, real ( kind = rk ) C(N), S(N), the cosines and sines of the
